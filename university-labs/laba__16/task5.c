...
void xor_encrypt(char *text, char *key, char *output) {
    ...
    for (int i = 0; i < text_len; i++) { // ошибка в условии, шифруется нул-терминатора
        output[i] = text[i] ^ key[i % key_len];
    }

    output[text_len] = '\0'; // добавил потому что на ее месте стоит какой-то мусор
}
